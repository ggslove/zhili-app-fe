...
interface IProps {
  ...
  labelTextChange: (event: React.FormEvent<HTMLInputElement | HTMLTextAreaElement>, newValue?: string) => void;
...
export default class LabelConfig extends  React.Component<IProps> {
  ...
  render() {
    const { build, width, colNumChange, labelTextChange, fontSizeChange, colorChange, rowConfig } = this.props;
        ...
        <Row>
          ...
          <Col md={colNum} className="config-col">
            <Row>
              ...
              <Col md={8}>
                <TextField value={rowConfig.labelText} onChange={labelTextChange}/>
              </Col>
            </Row>
          </Col>
          <Col md={colNum} className="config-col">
